...
(defun run-tests-error ()
  (let ((tests (parachute:status (cl-rm.test:run-tests))))
    (if (eq :failed tests)
        (error "tests failed")
        tests)))
...
(run-tests :designators '()) ==> run only those packages
```
"
  (test designators
  ...
  (let* ((packages
         ...
         (rm-packages
           (remove-if-not (lambda (p)
                            (let ((search (search "CL-RM" (package-name p))))
                              (and search (= 0 search))))
                          packages))
         (without-tests
             (remove-if (lambda (p)
                          (member (package-name p) '("CL-RM.TEST")
                                  :test #'equalp))
                        rm-packages)))
    (mapc (lambda (alu)
...
"
  ...
  (asdf:compile-system :cl-rm :force t)
  (asdf:compile-system cl-rm/test :force t)
    ...
    (mapc (lambda (test)
            ...
            (when test
              (setf (gethash test coverage)
                    (ccl:get-incremental-coverage))))
          (parachute:children (find-test 'cl-rm-test-suite)))
...
"
  ...
  (asdf:oos 'asdf:load-op :cl-rm :force t)
  (asdf:oos 'asdf:load-op :cl-rm/test :force t)
  (run-tests :summary? t)
